...
function questionDBRequest() {
  ...
  request.open("POST", "", true); //baseURL+'dbGetQuestion.php'
...
function displayQDB(response) {
  ...
  for (var i in questionDB2) {
    ...
    addcell.innerHTML =
      '<div class="text-center" ><input type="button" value="Add" onClick="addQuestion(' +
      i +
      ')" id="addingQuestion' +
      i +
      '"></div>';
...
function submitExam(event) {
  ...
  if (question_count == 0) {
    document.getElementById("status").innerHTML =
      "Nothing slected. Please select questions to Add to your exam";
...
function addQuestion(id) {
  ...
  new_div.innerHTML =
    ...
    question_name +
    `</label></div>
	<div><label id="label_points_` +
    id +
    `">Points: </label><input style ="width: 25px" type="text" id="points_` +
    actualID +
    `"></input></div>
	<div style="margin-top: ` +
    5 +
    `px"> <label><input onClick="deleteQuestion(` +
    id +
    `)" type="button" value="Remove" style="height: 20px; width: 25%"></label></div>
											</div><hr>`;
  ...
  document.getElementById("status").innerHTML =
    "Great! You are now creating an exam";
